        ...
        case KInputBoxStatusShowVoice:
        {
            if (fromStatus == KInputBoxStatusShowMore || fromStatus == KInputBoxStatusShowEmoji) {
                ...
                [UIView animateWithDuration:0.15 animations:^{
                    if (self->_delegate && [self->_delegate respondsToSelector:@selector(inputBoxCtrl:didChangeInputBoxHeight:)]) {
                        [self->_delegate inputBoxCtrl:self didChangeInputBoxHeight:kTabbarHeight];
                    }
                }];
            }
            else {
